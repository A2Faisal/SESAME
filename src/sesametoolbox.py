...
"""
def point_2_grid(shapefile_path, short_name, long_name, units="value/grid-cell", source=None, cell_size=1,
				 filename=None, time=None, zero_is_value=None, fold_field=None, fold_function="SUM",
				 attr_field=None, output_directory=None, output_filename=None, value_per_sqm=False, verbose=False,config=SesameConfig()):
	"""
	...
	if attr_field is not None:
		...
		dataset_list = get.get_unique_point_values(config, unique_values_list, shapefile_path, gdb_path, temp_path,
												   ...
												   cell_size, zero_is_value, value_per_sqm)
		
		ds = utils.merge_ds_list(config, dataset_list)
	else:
		raster_path = utils.point_2_tif(config, shapefile_path, temp_path, gdb_path, cell_size, filename,
...
#                     units = "value m-2"
#                 da = ds[short_name].to_array()
#                 ds = calculate.calculate_ds(config, value_per_sqm, da, short_name, long_name, units, source, cell_size, 
#                                             time, zero_is_value)
...
def line_2_grid(shapefile_path, short_name, long_name, units="km/grid-cell", source=None, cell_size=1.0,
	...
	if attr_field is not None:
		...
		dataset_list = get.get_unique_line_values(config, unique_values_list, shapefile_path, gdb_path, temp_path,
												  fold_field, fold_function, attr_field, units, source, time,
												  cell_size, zero_is_value)
		# Merge the list of datasets into one
		ds = utils.merge_ds_list(config, dataset_list)
	else:
		# Convert line-based shapefile to raster
		raster_path = utils.line_2_tif(config, shapefile_path, temp_path, gdb_path, cell_size, filename, fold_field,
									   fold_function)
		if short_name is None or long_name is None:
			short_name = base_filename
			long_name = process_data.reverse_replace_special_characters(short_name)
...
def poly_2_grid(shapefile_path, short_name=None, long_name=None, units="km2/grid-cell", source=None, cell_size=1.0,
	...
	if attr_field is not None:
		...
		raster_path = utils.poly_2_tif(config, shapefile_path, temp_path, gdb_path, cell_size, filename, fraction)
		if fraction is True:
			units = "fraction"
...
# TODO: be consistent with defualt units (sqm vs km2)
def table_2_grid(netcdf_variable, tabular_column, netcdf_file_path=None, csv_file_path=None, input_ds=None,
				 ...
				 filename=None, time=None, zero_is_value=None, value_per_sqm=None, verbose=False, config=SesameConfig()):
	"""
